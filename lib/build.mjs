...
import {
  ...
  stripForcedPanPlayerType,
  softenPanConcurrency,
} from "./sites.mjs";
...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  // 配置｜中心 → spider.jar（登录态）；夸父/阿离等跟 TOP 一样走全局 spider，不单独绑 jar
  sites = bindSpiderLoginJar(sites, spider);
  // 撤回误加的 Exo playerType（会导致网盘中途停缓存）
  sites = stripForcedPanPlayerType(sites);
  // 降并发，减轻网盘直链风控中断
  sites = softenPanConcurrency(sites, { maxThreads: 4 });
  sites = prioritizePanSearchSites(sites);
  onLog(
  ...
  const livesOut =
    Array.isArray(whMerge.lives) && whMerge.lives.length
      ? whMerge.lives
      : [{ name: "IPTV", type: 0, url: livesUrl, epg: "", logo: "" }];
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        ...(whMerge.danmaku ? { danmaku: whMerge.danmaku } : {}),
        sites,
        lives: livesOut,
        parses: whMerge.parses || [],
        flags: whMerge.flags || [],
        ...(whMerge.doh?.length ? { doh: whMerge.doh } : {}),
        ...(whMerge.rules?.length ? { rules: whMerge.rules } : {}),
        ijk: buildIjkOptions(whMerge.ijk),
        ads: [],
      },
      null,
      2,
