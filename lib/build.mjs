...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  // 播放/登录站共用 Config jar；夸快/盘搜等搜索站保留原 jar
  sites = hardenPanLoginSites(sites, spiderDownload);
  const panJar = sites.find((s) => /^csp_Config$/i.test(String(s.api || "")))?.jar;
  const kuaiJar = sites.find((s) => /^csp_PanWebShare$/i.test(String(s.api || "")))?.jar;
  if (panJar) {
    onLog(`[build] pan login jar -> ${String(panJar).split(";")[0]}`);
  }
  if (kuaiJar && kuaiJar !== panJar) {
    onLog(`[build] pan search jar (夸快) -> ${String(kuaiJar).split(";")[0]}`);
  }

  // 私有化：把所有 jar 下载到 publish/jar/，改写为 GitHub Pages 地址
  ...
  if (spider) {
    sites = alignPanSpiderJar(sites, spider);
    onLog(`[build] pan jar unified -> ${String(spider).split(";")[0]}`);
  }
