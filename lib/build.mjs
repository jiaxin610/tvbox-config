...
function resolveSpiderForHomeSite(sites, warehouseSpider = "") {
  ...
  // csp_* 首页走 API 映射；站点级 jar（如 pg.jar）只给该站点用，不当全局 spider
...
async function mirrorSpiderJar(spiderUrl, onLog) {
  ...
  const pagesBase = PAGES_BASE.replace(/\/$/, "");
  // 已是本站镜像地址时，直接读本地 jar，避免自引用下载
  if (src.startsWith(pagesBase)) {
    const rel = src.slice(pagesBase.length).replace(/^\//, "");
    const localPath = join(OUT, rel);
    try {
      const buf = await readFile(localPath);
      if (buf.length < 1000 || buf[0] !== 0x50 || buf[1] !== 0x4b) {
        throw new Error("not_jar");
      }
      const hosted = `${pagesBase}/${rel.replace(/\\/g, "/")}`;
      const md5 = createHash("md5").update(buf).digest("hex");
      onLog(`[jar] reuse local ${buf.length} bytes -> ${hosted}`);
      return `${hosted};md5;${md5}`;
    } catch (err) {
      onLog(`[jar] local reuse failed: ${err.message}`);
    }
  }
  try {
...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  // PG 盘搜（仅当 sources/csp-sites.json 存在；需「影视/OK影视/EasyBox」，其他壳会弹窗降级）
  const cspRaw = await loadCspSitesJson(join(ROOT, "sources", "csp-sites.json"));
  if (cspRaw.length) {
    const tokenmUrl = await hostTokenm({
      srcPath: join(ROOT, "sources", "tokenm.json"),
      outDir: OUT,
      pagesBase: PAGES_BASE,
      onLog,
    });
    const pgJar = await mirrorPgJar({
      outDir: OUT,
      pagesBase: PAGES_BASE,
      jarListPath: join(ROOT, "sources", "pg-jar.txt"),
      onLog,
    });
    const cspSites = prepareCspSites(cspRaw, {
      pgJar,
      tokenmUrl,
      prefix: "盘搜",
    });
    if (cspSites.length) {
      sites = injectCspSites(sites, cspSites);
      onLog(`[build] csp pan-search sites=${cspSites.length}`);
    } else {
      onLog(`[build] csp pan-search skipped (need pg.jar + tokenm)`);
    }
  } else {
    ...
    onLog(`[build] pg pan-search off (use TOP 盘搜/夸克云盘)`);
  }
