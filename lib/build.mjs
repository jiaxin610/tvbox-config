...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  let spider = isValidSpider(whMerge.spider) ? whMerge.spider : "";
  if (whMerge.spider && !spider) {
    onLog(`[build] spider cleared (invalid/local): ${whMerge.spider}`);
  }
  ...
  if (spider) {
    const mirrored = await mirrorSpiderJar(spider, onLog);
    spider = mirrored || "";
  }
