...
const JX_SPIDER = "https://szyyds.cn/tv/x.jpg";

/** 首页推荐站 api → 对应可下载的 jar 地址 */
const SPIDER_FOR_HOME_API = {
  csp_DouBan: DEFAULT_SPIDER,
  csp_DouDou: JX_SPIDER,
  csp_DouDouGuard: JX_SPIDER,
};
...
function resolveSpiderForHomeSite(sites) {
  ...
  if (/^csp_DouDou/i.test(api)) return JX_SPIDER;
...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  try {
    ...
    if (isDialPlaylistText(subText)) {
      localPlaylists.push({ name: "local", text: subText });
  ...
  // 有本地拨号源时：按 channels.txt 排序优先，其余频道也保留
  const wishlistOnly =
    localPlaylists.length > 0
      ? false
      : raw.wishlistOnly !== false;
