...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  // 原样私有化：只镜像 jar/资源到本仓库，不改 TOP 的 jar / Config / 盘搜结构
  ...
  onLog(
    `[build] privatized spider=${spider ? "yes" : "no"} panWebShare=${sites.filter((s) => /^csp_PanWebShare$/i.test(String(s.api || ""))).length} config=${sites.filter((s) => /^csp_Config$/i.test(String(s.api || ""))).length}`,
  );
