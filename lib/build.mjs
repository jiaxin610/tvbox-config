...
const SPIDER_FOR_HOME_API = {
  ...
  csp_DouBan: "http://home.jundie.top:81/jar/top98_1.jar",
};

function resolveSpiderForHomeSite(sites) {
  const home =
    (sites || []).find((s) => Number(s.indexs) === 1) ||
    (sites || []).find((s) => /豆瓣|DouBan|推荐/i.test(`${s.name}${s.key}`));
  if (!home) return "";
  const api = String(home.api || "").split("?")[0].trim();
  if (SPIDER_FOR_HOME_API[api]) return SPIDER_FOR_HOME_API[api];
  if (/^csp_/i.test(api)) return "http://home.jundie.top:81/jar/top98_1.jar";
  return "";
}
...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  let spiderDownload = resolveSpiderForHomeSite(sites);
  if (spiderDownload) {
    onLog(`[build] spider for home -> ${spiderDownload}`);
  } else if (isValidSpider(whMerge.spider)) {
    spiderDownload = String(whMerge.spider).split(";")[0].trim();
  ...
  onLog(
    `[build] subscribe=${mergedSub.length} cms=${cmsSites.length} vod=${sites.length} home=${sites[0]?.name || "-"} spider=${spider ? "yes" : "no"}`,
  );
