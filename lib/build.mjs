...
export async function buildPublish({ onLog = console.log } = {}) {
  ...
  onLog(
    `[build] vod=${sites.length} home=${sites[0]?.name || "-"} spider=${spider ? "yes" : "no"}`,
  );
    ...
    JSON.stringify(
      {
        ...
        home: sites[0]?.name || "",
        warehouses: warehouseEntries.map((w) => ({ name: w.name, url: w.url })),
...
<p>点播 ${sites.length} · 直播已关闭（用壳内搜台）</p>
<p>首页推荐：${(recommend.homeCategories || []).join(" · ")}</p>
<p>首页站点：${sites[0]?.name || "-"}</p>`,
    "utf8",
  );

  onLog(`[build] done vod=${sites.length} -> publish/`);
  return { channels: 0, sites: sites.length, livesUrl: "" };
}
