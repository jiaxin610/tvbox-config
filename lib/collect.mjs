...
function channelScore(ch, config) {
  ...
  if ((ch.urls?.[0] || "").startsWith("https")) score -= 1;
  return score;
}
...
export async function scanIptvSubscriptions(config, { onLog = () => {} } = {}) {
  ...
  candidates.sort(
    (a, b) => channelScore(a, config) - channelScore(b, config) || b.urls.length - a.urls.length,
  );
  ...
  aliveChannels.sort(compareChannels);

  if (aliveChannels.length > maxAliveChannels) {
    onLog(`trim channels ${aliveChannels.length} -> ${maxAliveChannels}`);
    // 已按 compareChannels 排序，保留前面的（CCTV1→2→…优先）
    aliveChannels.length = maxAliveChannels;
  }
