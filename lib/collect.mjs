...
/** 0=未知 1=标清/720 2=1080/高清 4=4K 8=8K */
export function detectQuality(name, url = "") {
  ...
  // 1080 / 高清 / 超清 / FHD / 蓝光（中文源里「高清」多为 1080）
  if (/1080p?|\bfhd\b|全高清|蓝光|超清|高清/i.test(nu)) return 2;
  if (/[\/_\-.](?:1080p?|fhd)[\/_\-.]/i.test(u)) return 2;
  ...
  // URL 里独立的 hd 片段（排除 hd 开头的域名如 hdcctv 已上面处理）
...
export async function scanIptvSubscriptions(config, { onLog = () => {} } = {}) {
  ...
  for (const ch of candidates) {
    for (const u of ch.urls) {
      if (minQuality > 0 && qualityOf(ch, u) < minQuality) continue;
