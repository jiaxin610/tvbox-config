...
/** 0=未知 1=高清 2=1080 4=4K 8=8K */
export function detectQuality(name, url = "") {
  const s = `${name || ""} ${url || ""}`;
  if (/8\s*k|4320p?/i.test(s)) return 8;
  if (/4\s*k|2160p?|uhd|超高清/i.test(s)) return 4;
  if (/1080p?|fhd|全高清/i.test(s)) return 2;
  if (/720p?|高清|\bhd\b/i.test(s)) return 1;
  return 0;
}
