...
/** 央视/卫视合并 4K，不再单独拆「4K央视」「4K卫视」 */
function unifiedGroup(ch) {
  const g = `${ch.group || ""} ${ch.name || ""}`;
  if (/卫视/i.test(g)) return "卫视";
  if (/CCTV|央视|中央/i.test(g)) return "央视";
  if (/卫视/i.test(ch.group || "")) return "卫视";
  if (/央视|CCTV/i.test(ch.group || "")) return "央视";
  return String(ch.group || "其他").replace(/^4K/i, "") || "其他";
}

/**
 * 有 4K 不要高清，有高清不要标清：只保留最高清晰度档
 * 档位：8=8K → 4=4K → 2=高清 → 1=标清
 */
function keepBestQualityTier(alive) {
  if (!alive?.length) return [];
  const best = Math.max(0, ...alive.map((a) => Number(a.quality) || 0));
  let floor = 0;
  if (best >= 8) floor = 8;
  else if (best >= 4) floor = 4;
  else if (best >= 2) floor = 2;
  else if (best >= 1) floor = 1;
  if (floor === 0) return alive;
  const kept = alive.filter((a) => (Number(a.quality) || 0) >= floor);
  return kept.length ? kept : alive;
}
...
export async function scanIptvSubscriptions(config, { onLog = () => {} } = {}) {
  ...
  for (const ch of candidates) {
    ...
    alive = keepBestQualityTier(alive).slice(0, keepPerChannel);
