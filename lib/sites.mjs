...
/**
 * 夸克/阿里登录态跟全局 spider（与 TOP 一致：Pan* 不单独写 jar）。
 * 只给「无 jar 的配置中心」绑 spider.jar。
 */
 ...
 * 推送站靠前，方便把解析地址发到手机用 IDM+ 下载。
 * 不改 playerType（TOP 原版不设，强行 Exo 会导致中途停缓存）。
 */
export function prioritizePanSearchSites(sites) {
  ...
  for (const s of sites || []) {
    ...
    if (/^csp_Push/i.test(api)) {
      pushes.push(s);
      continue;
    }
    rest.push(s);
  }
  return [...home, ...configs, ...pans, ...pushes, ...rest];
}

/** 去掉误加的网盘 playerType，避免强制 Exo 导致停缓存 */
export function stripForcedPanPlayerType(sites) {
  const panRe =
    /^csp_(?:PanWebShare|PanSou|PanSearch|Duopan|PanAli|PanQuark|PanUC|PanBaidu|WoGG)$/i;
  return (sites || []).map((s) => {
    const api = String(s.api || "").split("?")[0];
    if (!panRe.test(api)) return s;
    if (s.playerType == null) return s;
    const { playerType, ...rest } = s;
    return rest;
  });
}

/**
 * 降低网盘多线程，减轻夸克/阿里并发风控导致的「播到一半突然不加载」。
 * 同时去掉副仓带来的 timeout 覆盖，跟 TOP 一致。
 */
export function softenPanConcurrency(sites, { maxThreads = 4 } = {}) {
  const panRe =
    /^csp_(?:PanWebShare|PanSou|PanSearch|Duopan|PanAli|PanQuark|PanUC|PanBaidu|WoGG)$/i;
  return (sites || []).map((s) => {
    const api = String(s.api || "").split("?")[0];
    ...
    let next = s;
    if (Object.prototype.hasOwnProperty.call(next, "timeout")) {
      const { timeout, ...rest } = next;
      next = rest;
    }

    const ext = next.ext;
    if (!ext || typeof ext !== "object" || Array.isArray(ext)) return next;

    const info = ext.threadinfo;
    if (!info || typeof info !== "object") return next;
    const threads = Number(info.threads);
    if (!Number.isFinite(threads) || threads <= maxThreads) return next;

    return {
      ...next,
      ext: {
        ...ext,
        threadinfo: { ...info, threads: maxThreads },
      },
    };
  });
}
