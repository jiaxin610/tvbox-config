...
/**
 * 网盘播放站：用 Exo(playerType=1) 更利于断线续传；多线程拉流略增稳。
 */
export function hardenPanPlayback(sites) {
  return (sites || []).map((s) => {
    const api = String(s.api || "").split("?")[0].trim();
    if (
      !/^csp_(?:PanWebShare|PanSou|PanSearch|Duopan|PanAli|PanQuark|PanUC|PanBaidu|WoGG|Push)/i.test(
        api,
      )
    ) {
      return s;
    ...
    const site = { ...s };
    if (site.playerType == null) site.playerType = 1;
    if (site.timeout == null) site.timeout = 60;

    if (/^csp_Duopan$/i.test(api) && site.ext && typeof site.ext === "object") {
      const ext = { ...site.ext };
      const ti = { ...(ext.threadinfo || {}) };
      if (!ti.threads || Number(ti.threads) < 8) ti.threads = 16;
      if (!ti.chunksize) ti.chunksize = 512;
      ext.threadinfo = ti;
      site.ext = ext;
    }
    return site;
