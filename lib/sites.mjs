...
/** 需与扫码 Config 共用 jar 的播放站（登录态互通） */
const PAN_LOGIN_JAR_API_RE =
  /^csp_(?:Config|PanAli|PanQuark|PanUC|PanBaidu|Duopan|WoGG|Push)$/i;
...
export function needsSharedPanLoginJar(site) {
  const api = String(site?.api || "").split("?")[0].trim();
  if (PAN_SEARCH_API_RE.test(api)) return false;
  if (PAN_LOGIN_JAR_API_RE.test(api)) return true;
  if (/^csp_Pan/i.test(api)) return true;
  return false;
}

/**
 * 强制网盘站 changeable=0；扫码配置站提前；
 * 登录态存在 jar 内：优先用 global spider（0820），与夸快播放同源。
 */
...
/** 网盘播放/扫码/盘搜聚合：强制与全局 spider 同一 jar（登录态才能带到播放） */
const PAN_UNIFIED_JAR_API_RE =
  /^csp_(?:Config|Pan|Duopan|WoGG|Wobg|Push)/i;

export function alignPanSpiderJar(sites, spiderField) {
  const jar = String(spiderField || "").trim();
  if (!jar) return sites || [];
  return (sites || []).map((s) => {
    const api = String(s.api || "").split("?")[0];
    if (PAN_UNIFIED_JAR_API_RE.test(api)) {
      return { ...s, jar };
    }
    return s;
  });
