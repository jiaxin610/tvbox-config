...
export function prioritizePanSearchSites(sites) {
  ...
  for (const s of sites || []) {
    ...
    if (/^csp_(?:PanWebShare|PanSou|PanSearch|Duopan)$/i.test(api)) {
      ...
      // 盘搜/解析偶发慢，超时太短会表现为「突然不加载」
      if (pan.timeout == null || Number(pan.timeout) < 60) pan.timeout = 60;
...
export function stripForcedPanPlayerType(sites) {
  ...
  return (sites || []).map((s) => {
    ...
    let next = s;
    if (s.playerType != null) {
      const { playerType, ...rest } = s;
      next = rest;
    }
    // 播放解析超时略放宽，减少中途拉流失败直接停住
    if (next.timeout == null || Number(next.timeout) < 60) {
      next = { ...next, timeout: 60 };
    }
    return next;
  });
}
