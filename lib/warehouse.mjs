  ...
  for (const { name, url } of entries) {
    try {
      ...
      if ((!ijk || !ijk.length) && Array.isArray(cfg.ijk) && cfg.ijk.length) {
        ijk = cfg.ijk;
        onLog(`ijk -> ${cfg.ijk.map((g) => g.group).join(",")}`);
      }
      if ((!doh || !doh.length) && Array.isArray(cfg.doh) && cfg.doh.length) {
        doh = cfg.doh;
        onLog(`doh -> ${cfg.doh.length}`);
      }
      if (Array.isArray(cfg.rules) && cfg.rules.length) {
        // 副仓嗅探/广告规则：按 name|host 去重后追加
        const seen = new Set(
          rules.map((r) => `${r?.name || ""}|${r?.host || ""}|${JSON.stringify(r?.hosts || [])}`),
        );
        for (const r of cfg.rules) {
          if (!r || typeof r !== "object") continue;
          const k = `${r.name || ""}|${r.host || ""}|${JSON.stringify(r.hosts || [])}`;
          if (seen.has(k)) continue;
          seen.add(k);
          rules.push(r);
        }
        onLog(`rules += ${cfg.rules.length} (total ${rules.length})`);
      }
